...
public class FormDef implements IFormElement, Localizable, Persistable, IMetaData,
    ...
    public TreeReference getChildInstanceRef(List<IFormElement> elements,
                                             List<Integer> multiplicities) {
        String entry = newDebugMsg(null, multiplicities, elements);
        Timber.i("getChildInstanceRef:+ " + entry);
        ...
        String exit = newDebugMsg(null, multiplicities, elements);
        Timber.i("getChildInstanceRef_:+ "
                + (entry.equals(exit) ? ref : exit));
        return ref;
    }

    public FormIndex buildIndex(List<Integer> indexes,
                                List<Integer> multiplicities,
                                List<IFormElement> elements) {
        Timber.i("buildIndex:+ "
                + newDebugMsg(indexes, multiplicities, elements));
        FormIndex cur = null;
        ...
        String treeText = mi.checkTreeText(true);
        Timber.i("buildIndex_:+" + cur + " " + treeText);
        return cur;
    }
    ...
    public void collapseIndex(FormIndex index,
        ...
        Timber.i("collapseIndex:+ " +
                (true ? index : newDebugMsg(indexes, multiplicities, elements)));
    ...
    public TreeReference getChildInstanceRef(FormIndex index) {
        Timber.i("getChildInstanceRef:+ " + index);
        List<Integer> indexes = new ArrayList<>();
        List<Integer> multiplicities = new ArrayList<>();
        List<IFormElement> elements = new ArrayList<>();
        ...
        collapseIndex(index, indexes, multiplicities, elements);
        TreeReference ref = getChildInstanceRef(elements, multiplicities);
        Timber.i("getChildInstanceRef_:+ " + ref);
        return ref;
    }
    ...
    public void setValue(IAnswerData data, TreeReference ref, boolean midSurvey) {
        setValue(data, ref, mi.resolveReference(ref), midSurvey);
    }
    ...
    public void setAnswer(IAnswerData data, TreeReference ref) {
        setAnswer(data, mi.resolveReference(ref));
    }
    ...
    public FormIndex deleteRepeat(FormIndex index) {
        ...
        TreeElement deleteElement = mi.resolveReference(deleteRef);
        TreeReference parentRef = deleteRef.getParentRef();
        TreeElement parentElement = mi.resolveReference(parentRef);
        ...
        Timber.i("createNewRepeat:+ " + newIndex);
        return newIndex;
    }

    public void createNewRepeat(FormIndex index) throws InvalidReferenceException {
        String _tree0 = mi.checkTreeText(false);
        Timber.i("createNewRepeat:+ " + index + " " + _tree0);

        TreeReference destRef = getChildInstanceRef(index);
        TreeElement template = mi.getTemplate(destRef);
        String _template = newTreeText(template);

        //Fix for #4059?
        boolean for_4059 = true;
        if (false && for_4059) {
            mi.copyNodeForRepeat(template, destRef);
        } else {
            mi.copyNode(!for_4059 ? template
                    : false ? deepCopyForRepeat(template)
                    : template.deepCopyForRepeat(), destRef);
        }
        String _tree1 = mi.checkTreeText(false);

        TreeElement newNode = mi.resolveReference(destRef);
        String _newNode = newTreeText(newNode);
        ...
        actionController.triggerActionsFromEvent(Actions.EVENT_JR_INSERT, this, destRef, this);
        actionController.triggerActionsFromEvent(Actions.EVENT_ODK_NEW_REPEAT, this, destRef, this);
        // Trigger actions nested in the new repeat
        getChild(index).getActionController().triggerActionsFromEvent(Actions.EVENT_ODK_NEW_REPEAT, this, destRef, this);

        dagImpl.createRepeatInstance(getMainInstance(), getEvaluationContext(), destRef, newNode);
        Timber.i("createNewRepeat_:+ " + index);
    }

    //#4059
    private TreeElement deepCopyForRepeat(TreeElement node) {
        String _this = newTreeText(node);
        TreeElement newNode = node.shallowCopy();
        String _newNode0 = newTreeText(newNode);

        ArrayList<TreeElement> newChildren = new ArrayList<>();
        int numChildren = newNode.getNumChildren();
        for (int at = 0; at < numChildren; at++) {
            TreeElement child = newNode.getChildAt(0);
            newChildren.add(child);
            newNode.removeChildAt(0);
            String _newNode1 = newTreeText(newNode);
        }
        for (TreeElement child : newChildren) {
            child.setMult(TreeReference.DEFAULT_MULTIPLICITY);
            newNode.addChild(deepCopyForRepeat(child));
            String _newNode1 = newTreeText(newNode);
        }
        String _newNode2 = newTreeText(newNode);
        return newNode;
    }
    ...
    public boolean isRepeatRelevant(TreeReference repeatRef) {
        ...
        if (qc != null) {
            relev = (boolean) qc.eval(mi, new EvaluationContext(exprEvalContext, repeatRef));
        }

        if (relev) {
            // Must check the template in case of static relevance expressions that wouldn't be in the DAG (e.g. false()).
            TreeElement templNode = mi.getTemplate(repeatRef);
    ...
    public boolean evaluateConstraint(TreeReference ref, IAnswerData data) {
        ...
        TreeElement node = mi.resolveReference(ref);
        ...
        boolean result = c.constraint.eval(mi, ec);
    ...
    private void resetEvaluationContext() {
        ...
        ec = new EvaluationContext(mi, getFormInstances(), ec);
        initEvalContext(ec);
    ...
    public boolean postProcessInstance() {
        ...
        return postProcessInstance(mi.getRoot());
    }
    ...
    public void readExternal(DataInputStream dis, PrototypeFactory pf) throws IOException,
        ...
        mi.getBase().setInstanceName(null);
    ...
    public int getNumRepetitions(FormIndex index) {
        ...
        // so painful
        TreeElement templNode = mi.getTemplate(index.getReference());
        TreeReference parentPath = templNode.getParent().getRef().genericize();
        TreeElement parentNode = mi.resolveReference(parentPath.contextualize(index
                .getReference()));
    ...
    public String getMetaData(String fieldName) {
        ...
        if (fieldName.equals("XMLNS")) {
            return ExtUtil.emptyIfNull(mi.schema);
        } else {
