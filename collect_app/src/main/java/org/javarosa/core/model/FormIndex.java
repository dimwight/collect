...
public class FormIndex implements Serializable {
    ...
    public String toString() {
        ...
        FormIndex lowest = this;
        TreeReference reference = this.reference;
        String path = reference == null ? "" : reference.toString();
        while (lowest != null) {
            b.append(lowest.getLocalIndex());
            if (lowest.getInstanceIndex() != -1) {
                b.append("_").append(lowest.getInstanceIndex());
            }
            if (!path.isEmpty()) {
                path = lowest.reference.toString().replace("/data", "");
            }
            b.append(", ");
            lowest = lowest.nextLevel;
        }
        return !path.isEmpty() ? path.replace("[1]", "")
                : b.toString();
    }
