...
public abstract class DataInstance<T extends AbstractTreeElement<T>> implements Persistable {
    ...
    private String thenTree = "";
    static DataInstance thisThen;

    public T resolveReference(TreeReference ref) {
        boolean newThis = this != thisThen;
        Timber.i("resolveReference:+ " + (newThis ? "!" : "")
                + ref.toString().replace("/data", ""));
        thisThen = this;

        if (!ref.isAbsolute()) {
            Timber.i("resolveReference_:+ null");
            return null;
        }
        ...
        T back = node == getBase() ? null : result;
        String treeText = checkTreeText(true);
        Timber.i("resolveReference_:+ " +
                (!treeText.isEmpty() ? treeText : back == null ? "null"
                        : result.toString()
                        .replace(" - Children: ", "+")));
        return back; // never return a reference to '/'
    }

    public String checkTreeText(boolean checkChanged) {
        return checkTreeText(null, checkChanged);
    }

    public String checkTreeText(TreeElement root, boolean checkChanged) {
        TreeElement node = root != null ? root : (TreeElement) getRoot();
        String nowTree = newTreeText(node)
                .replaceAll("^.*\\Q" +
                        "data:0 {person:-2 {age:0 child:-2 {name:0}} "
                        + "\\E", "…")
                .replaceAll("\\}$", "");
        if (root != null) return nowTree;
        boolean newTree = !nowTree.equals(thenTree);
        if (!nowTree.isEmpty()) {
            thenTree = nowTree;
        }
        return !checkChanged || newTree ? nowTree : "";
    }

    public static String newTreeText(TreeElement root) {
        StringBuilder sb = new StringBuilder();
        expandTreeNode(sb, root);
        return sb.toString();
    }

    private static void expandTreeNode(StringBuilder sb, TreeElement node) {
        int num = node.getNumChildren();
        sb.append("")
                .append(node.getName())
                .append(":")
                .append(node.getMultiplicity()).append("");
        if (num > 0) {
            sb.append(" {");
            for (int at = 0; at < num; at++) {
                expandTreeNode(sb, node.getChildAt(at));
                if (at < num - 1) {
                    sb.append(" ");
                }
            }
    ...
    public T getTemplate(TreeReference ref) {
        Timber.i("getTemplate:+ " + ref);
        T node = getTemplatePath(ref);
        T get = node == null ? null : ((node.isRepeatable() || node.isAttribute()) ? node : null);
        Timber.i("getTemplate_:+ " + get);
        return get;
    }
