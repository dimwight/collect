...
public class FormInstance extends DataInstance<TreeElement> implements Persistable, IMetaData {
    ...
    //For #4059
    public TreeElement copyNodeForRepeat(TreeElement src,
                                         TreeReference destRef)
            throws InvalidReferenceException {
        if (!destRef.isAbsolute())
            throw new InvalidReferenceException("Destination reference must be absolute for copying", destRef);
        String _src = newTreeText(src);
        TreeReference parentRef = destRef.getParentRef();
        TreeElement parent = resolveReference(parentRef);
        String _parent0 = newTreeText(parent);
        if (parent == null) {
        ...
        int destMult = destRef.getMultLast();
        String destName = destRef.getNameLast();
        if (destMult == INDEX_UNBOUND) {
            destMult = parent.getChildMultiplicity(destName);
        } else if (parent.getChild(destName, destMult) != null) {
            throw new InvalidReferenceException("Destination already exists!", destRef);
        }
        //For #4059
        TreeElement dest = src.deepCopyForRepeat();
        dest.setName(destName);
        String _dest0 = newTreeText(dest);
        dest.setMult(destMult);
        String _dest1 = newTreeText(dest);
        parent.addChild(dest);
        String _parent1 = newTreeText(parent);
        return dest;
    }
    ...
    private TreeElement createNode(TreeReference ref) {
        ...
        for (int k = 0; k < ref.size(); k++) {
            ...
            if (k < ref.size() - 1) {
                ...
                if (mult == INDEX_UNBOUND || mult == count) {
